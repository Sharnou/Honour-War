...
# Art path: Visual RAG -> Blender/Neural4D -> Substance 3D Painter handoff -> FBX/OBJ -> Unreal Engine 5.8.
...
def build_all():
    ...
    # Heroes: exact runtime naming contract, 30 class/tier FBX assets.
    ...
    for species in ["Falcon", "Wolf", "Panther", "PoringAngel", "ArcaneOrb", "Clockwork"]:
        ...
        export_root(root, os.path.join(ROOT, "pets", species + ".fbx"))
    clear()
    with open(os.path.join(ROOT, "VISUAL_MAX_BUILD.txt"), "w", encoding="utf-8") as f:
        ...
        f.write("30 class/tier hero FBX assets + class pet FBX assets + shared pet FBX assets.\n")
