...
if not defined GODOT if exist "%PROJECT%\Godot_v4.2.2-stable_win64.exe" set "GODOT=%PROJECT%\Godot_v4.2.2-stable_win64.exe"
if not defined GODOT if exist "%PROJECT%\Godot_v4.2.2-stable_win64_console.exe" set "GODOT=%PROJECT%\Godot_v4.2.2-stable_win64_console.exe"
...
rem The launcher already runs from the project directory. Passing '.' avoids
rem Windows/Godot argument parsing issues with quoted absolute project paths.
...
:CONSOLE_START
start "Honour War" /b "" "%GODOT%" --path . --rendering-method gl_compatibility --rendering-driver opengl3 %*
set "RESULT=%errorlevel%"
