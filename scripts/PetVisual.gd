...
var role_color:Color=Color("#d8c48b")
...
func _draw()->void:
	...
	var center:Vector2=Vector2(0,bob)
	var species:String=str(pet.get("species","Pet"))
	var scale_factor:float=1.0+min(0.25,float(level)/400.0)
	if species=="Royal Falcon":
		draw_circle(center,12.0*scale_factor,role_color)
		...
		draw_circle(center,13.0*scale_factor,role_color)
		draw_circle(center,20.0*scale_factor,Color(role_color,0.22))
		draw_circle(center+Vector2(-5,-3),2.0,Color.WHITE)
		draw_circle(center+Vector2(5,-3),2.0,Color.WHITE)
	elif species=="Blessed Poring":
		draw_circle(center,15.0*scale_factor,role_color)
		draw_line(center+Vector2(-7,-2),center+Vector2(-3,3),Color("#563d68"),2.0)
		draw_line(center+Vector2(7,-2),center+Vector2(3,3),Color("#563d68"),2.0)
	elif species=="Night Panther" or species=="Dire Wolf":
		draw_circle(center,14.0*scale_factor,role_color)
	...
	var hp_ratio:float=clamp(float(pet.get("hp",60))/max(1.0,float(pet.get("max_hp",60))),0.0,1.0)
