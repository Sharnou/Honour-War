...
func _hero_attack_point(target_pos:Vector2)->void:
    ...
    _spawn_attack_arc(origin,target_pos,origin.direction_to(target_pos),class_id,false)
...
func _spawn_attack_arc(origin:Vector2,target:Vector2,direction:Vector2,class_id:String,pet:bool)->void:
    ...
    arc.rotation_degrees=Vector3(90.0,yaw,18.0 if pet else 0.0)
    ...
    line_tween.tween_property(line,"scale",Vector3(1.0,1.0,0.15),0.16)
