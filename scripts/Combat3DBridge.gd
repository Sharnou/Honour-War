...
const DAMAGE_NUMBERS_SCRIPT=preload("res://scripts/DamageNumbers3D.gd")
var numbers:Node3D
...
func show_number(map_position:Vector2,amount:int,critical:bool=false,source:String="enemy")->void:
	...
	if source=="enemy" and legacy!=null:
		...
		if hero_value is Dictionary:
			...
			if hero_pos.distance_to(map_position)<28.0: source="hero"
	var game:=get_parent().get_parent()
	var world_position:=Vector3((map_position.x-365.0)*0.055,0.0,(map_position.y-120.0)*0.055)
	if numbers.has_method("show_damage"):
		numbers.call("show_damage",game.to_global(world_position),amount,critical,source)
