...
func _character(current:Dictionary)->void:
	_clear_body()
	_add_heading("CHARACTER / LIVE PROGRESSION")
	var growth:Dictionary=AGE.strength_bonus(current)
	var stats:Dictionary=CHARACTER.stats(current)
	var progress:Dictionary=CHARACTER.xp_progress(current)
	var power:int=CHARACTER.combat_power(current)
	var pet:Dictionary=current.get("pet",{}) if current.get("pet",{}) is Dictionary else {}
	var label:=Label.new()
	label.text="Level %d / 250   •   %s   •   Combat Power %d\nXP %d / %d  •  %.1f%%\nStat Points: %d\nAge %d • %s • %.2f online days\nZeny: %d\n\nATK %d  MATK %d\nDEF %d  MDEF %d\nHP %d / %d   SP %d / %d\nCRIT %.1f  HIT %d  FLEE %d\nHealing %d\n\nAGE BONUS\nATK +%d  MATK +%d  DEF +%d  MDEF +%d\nHP +%d  SP +%d  CRIT +%d  HIT +%d  FLEE +%d\n\nPET\n%s  Lv.%d  XP %d" % [int(current.get("level",1)),str(current.get("class","Warrior")),power,int(progress["xp"]),int(progress["next"]),float(progress["ratio"])*100.0,int(current.get("stat_points",0)),int(current.get("age",18)),AGE.title(int(current.get("age",18))),float(current.get("online_days",0.0)),int(current.get("zeny",0)),int(stats.get("atk",0)),int(stats.get("matk",0)),int(stats.get("def",0)),int(stats.get("mdef",0)),int(current.get("hp",stats.get("max_hp",0))),int(stats.get("max_hp",0)),int(current.get("sp",stats.get("max_sp",0))),int(stats.get("max_sp",0)),float(stats.get("crit",0)),int(stats.get("hit",0)),int(stats.get("flee",0)),int(stats.get("healing",0)),int(growth["atk"]),int(growth["matk"]),int(growth["def"]),int(growth["mdef"]),int(growth["hp"]),int(growth["sp"]),int(growth["crit"]),int(growth["hit"]),int(growth["flee"]),str(pet.get("name","None")),int(pet.get("level",1)),int(pet.get("xp",0))]
	label.autowrap_mode=TextServer.AUTOWRAP_WORD_SMART
	body.add_child(label)
	_add_heading("STAT ALLOCATION • 99 CAP EACH")
	for stat in CHARACTER.STAT_NAMES:
		var row:=HBoxContainer.new()
		row.size_flags_horizontal=Control.SIZE_EXPAND_FILL
		body.add_child(row)
		var stat_label:=Label.new()
		stat_label.text="%s  %d / 99" % [stat.to_upper(),int(current["stats"].get(stat,1))]
		stat_label.size_flags_horizontal=Control.SIZE_EXPAND_FILL
		row.add_child(stat_label)
		var one:=Button.new(); one.text="+1"; one.disabled=int(current.get("stat_points",0))<=0 or int(current["stats"].get(stat,1))>=99; one.pressed.connect(_allocate_stat.bind(stat,1)); row.add_child(one)
		var five:=Button.new(); five.text="+5"; five.disabled=int(current.get("stat_points",0))<=0 or int(current["stats"].get(stat,1))>=99; five.pressed.connect(_allocate_stat.bind(stat,5)); row.add_child(five)
	_add_button("RESET ALL STATS",Callable(self,"_reset_stats"))
...
func _allocate_stat(stat:String,amount:int)->void:
	...
	var ok:bool=CHARACTER.allocate(hero,stat,amount)
	if ok:
		var after:Dictionary=CHARACTER.stats(hero)
		_set_status("✓ %s +%d • ATK %d→%d • HP %d→%d • Stat Points %d" % [stat.to_upper(),amount,int(before["atk"]),int(after["atk"]),int(before["max_hp"]),int(after["max_hp"]),int(hero.get("stat_points",0))])
	else: _set_status("✕ No stat points available or stat is capped.")
	timer=1.0
...
func _inventory(current:Dictionary)->void:
	...
	economy.text="Zeny: %d   •   HP %d/%d   •   SP %d/%d   •   Weight/Cargo +%d" % [int(current.get("zeny",0)),int(current.get("hp",0)),int(stats.get("max_hp",0)),int(current.get("sp",0)),int(stats.get("max_sp",0)),int(stats.get("carry",0))]
...
func _use_item(item_id:String)->void:
	var result:Dictionary=CHARACTER_INV.use_consumable(hero,item_id); _show_result(result,"Used "+item_id); timer=1.0

func _equipment(current:Dictionary)->void:
	_clear_body(); _add_heading("EQUIPMENT • LIVE CHARACTER STATS")
	var stats:Dictionary=CHARACTER.stats(current)
	var stat_box:=Label.new(); stat_box.text="Combat Power %d\nATK %d   MATK %d   DEF %d   MDEF %d\nHP %d/%d   SP %d/%d   CRIT %.1f" % [CHARACTER.combat_power(current),int(stats["atk"]),int(stats["matk"]),int(stats["def"]),int(stats["mdef"]),int(current.get("hp",0)),int(stats["max_hp"]),int(current.get("sp",0)),int(stats["max_sp"]),float(stats["crit"]))]; body.add_child(stat_box)
	for slot in ["weapon","armor","head","head_middle","head_lower","garment","shoes","offhand","accessory_1","accessory_2"]:
		var equipped:Variant=current.get("equipment",{}).get(slot,null); var name:String="Empty"; var refine:int=0
		if equipped is Dictionary: name=str(equipped.get("id",equipped.get("name","Unknown"))); refine=int(equipped.get("refine",current.get("equipment_refine",{}).get(slot,0)))
	...
	if selected_slot!="":
		_add_heading("SELECTED SLOT: "+selected_slot.to_upper()); _add_button("REFINE SELECTED SLOT",Callable(self,"_refine_selected")); _add_button("CHOOSE A CARD FOR THIS SLOT",Callable(self,"_show_card_picker")); if selected_card!="": _add_button("INSERT SELECTED CARD: "+selected_card,Callable(self,"_insert_selected_card"))
...
func _refine(current:Dictionary)->void:
	_clear_body(); _add_heading("REFINEMENT")
	for slot in ["weapon","armor","head","head_middle","head_lower","garment","shoes","offhand","accessory_1","accessory_2"]:
		if not current.get("equipment",{}).has(slot): continue
		var item:Dictionary=current["equipment"][slot] if current["equipment"][slot] is Dictionary else {}; var refine:int=int(item.get("refine",current.get("equipment_refine",{}).get(slot,0))); var chance:float=EQUIPMENT.refine_chance(refine); var material:String="Oridecon" if refine>=5 else "Phracon"; var owned:int=int(current.get("inventory",{}).get(material,0)); if current.get("inventory",{}).get(material,0) is Dictionary: owned=int(current["inventory"][material].get("amount",0))
		var row:=HBoxContainer.new(); body.add_child(row); var label:=Label.new(); label.text="%s  +%d → +%d  •  %.0f%%  •  %s x%d" % [str(item.get("id","Item")),refine,refine+1,chance*100.0,material,owned]; label.size_flags_horizontal=Control.SIZE_EXPAND_FILL; row.add_child(label); var button:=Button.new(); button.text="REFINE"; button.disabled=owned<=0; button.pressed.connect(_refine_slot.bind(slot)); row.add_child(button)
...
func _show_card_picker()->void:
	_clear_body(); _add_heading("CARD SELECTOR • SELECT AN OWNED CARD")
	var cards_value:Variant=hero.get("cards",[]); var cards:Array=cards_value if cards_value is Array else []
	if cards.is_empty(): var empty:=Label.new(); empty.text="No cards owned."; body.add_child(empty); _add_button("BACK TO EQUIPMENT",Callable(self,"_set_mode").bind("equipment")); return
	for card_value in cards:
		var card_id:String=str(card_value); var button:=Button.new(); button.text=card_id; button.pressed.connect(_select_card.bind(card_id)); body.add_child(button)
	if selected_card!="": _add_heading("SELECTED CARD: "+selected_card); _add_button("INSERT INTO "+selected_slot.to_upper(),Callable(self,"_insert_selected_card"))
	_add_button("BACK TO EQUIPMENT",Callable(self,"_set_mode").bind("equipment"))
func _select_card(card_id:String)->void: selected_card=card_id; mode="equipment"; timer=1.0
...
func _set_status(text:String)->void: status_label.text=text
