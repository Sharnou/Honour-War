...
func _build_ui()->void:
	...
	panel.position=Vector2(1410,110)
	...
	var title:=Label.new()
	title.text="HONOUR WAR  •  CHARACTER / WORLD SYSTEMS"
	...
	root.add_child(body)
	var hint:=Label.new()
	hint.text="I = Inventory   E = Events   M = Monster details   C = Character"
	root.add_child(hint)
...
func _character(hero:Dictionary)->void:
	var age:int=int(hero.get("age",18))
	var growth:Dictionary=AGE.strength_bonus(hero)
	body.text="[b]AGE: %d years[/b]  •  %s\nOnline time: %.2f days\n\n[b]UNLIMITED AGE GROWTH[/b]\nEvery 3 online days = +1 year. Age has no maximum.\n\nATK +%d   MATK +%d\nDEF +%d   MDEF +%d\nHP +%d    SP +%d\nCRIT +%d  HIT +%d  FLEE +%d\nHealing +%d\n\nLevel %d / 250\nClass: %s\nKills: %d" % [age,AGE.title(age),growth["atk"],growth["matk"],growth["def"],growth["mdef"],growth["hp"],growth["sp"],int(growth["crit"]),int(growth["hit"]),int(growth["flee"]),int(growth["healing"]),int(hero.get("level",1)),str(hero.get("class","Warrior")),int(hero.get("kills",0))]
