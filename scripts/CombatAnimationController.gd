...
func _make_walk(is_pet:bool)->Animation:
    ...
    var base:=0.45 if is_pet else 0.15
...
func _process(delta:float)->void:
    ...
    if hero_node!=null:
        ...
        if hero_pulse>0.0:
            hero_node.scale*=1.0+hero_pulse*0.10
        if hit_pulse>0.0 and hero_player.current_animation!=\"hit\":
            hero_node.scale*=1.0+hit_pulse*0.04
        last_hero_pos=hero_pos
    if pet_node!=null and pet_attack_lock<=0.0:
        if pet_pulse<=0.0:
            if pet_player.current_animation!=\"idle\" and pet_player.current_animation!=\"walk\":
