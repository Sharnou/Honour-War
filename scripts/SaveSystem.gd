...
static func load_game(default_hero:Dictionary) -> Dictionary:
	if not FileAccess.file_exists(SAVE_PATH): return default_hero.duplicate(true)
	var file=FileAccess.open(SAVE_PATH, FileAccess.READ)
	if file == null: return default_hero.duplicate(true)
	var parsed=JSON.parse_string(file.get_as_text())
	file.close()
	if typeof(parsed) != TYPE_DICTIONARY: return default_hero.duplicate(true)
