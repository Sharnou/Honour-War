...
## Screen-readable combat vitals. Uses simple 3D bars so health remains visible
## without world-space nameplates or text clutter.
var game:Node3D
var legacy:Node
...
func _update_hero()->void:
    ...
    _set_fill(hero_hp_fill,hp/max_hp)
    _set_fill(hero_sp_fill,sp/max_sp)
...
func _build_hero_bars(hero:Node3D)->void:
    ...
    hero.add_child(root)
    var hp_bg:MeshInstance3D=_bar(Color("#241217"),Vector3(1.15,0.07,0.02))
    hp_bg.position=Vector3(0,0,0)
    root.add_child(hp_bg)
    hero_hp_fill=_bar(Color("#d74d59"),Vector3(1.11,0.055,0.023))
    hero_hp_fill.position=Vector3(-0.02,0,-0.02)
    root.add_child(hero_hp_fill)
    var sp_bg:MeshInstance3D=_bar(Color("#10192b"),Vector3(1.15,0.055,0.02))
    sp_bg.position=Vector3(0,-0.13,0)
    root.add_child(sp_bg)
    hero_sp_fill=_bar(Color("#4f8de1"),Vector3(1.11,0.045,0.023))
...
func _build_monster_bar(monster:Node3D,boss:bool)->Dictionary:
    ...
    var bg:MeshInstance3D=_bar(Color("#210f15"),Vector3(0.92 if not boss else 1.35,0.065,0.02))
    root.add_child(bg)
    var fill:MeshInstance3D=_bar(Color("#cf4451"),Vector3(0.88 if not boss else 1.31,0.052,0.023))
    fill.position.z=-0.02
    root.add_child(fill)
    _billboard(root)
    return {"root":root,"hp":fill}

func _set_fill(fill:MeshInstance3D,ratio:float)->void:
    if fill==null or not is_instance_valid(fill):
        return
    var r:float=clamp(ratio,0.0,1.0)
    var full:Vector3=fill.scale
    var base_width:float=1.11
    if fill.get_meta("boss_bar",false):
        base_width=1.31
    fill.scale.x=max(0.02,r)
    fill.position.x=-((base_width)*(1.0-r))*0.5
...
func _billboard(node:Node3D)->void:
    var camera:Camera3D=game.get_node_or_null("Camera3D") as Camera3D
    if camera==null:
        return
    var script:String=""
    var helper:Node3D=node
    helper.look_at(camera.global_position,Vector3.UP)
    node.rotation.x=0.0
    node.rotation.z=0.0
...
func _add_class_features(hero:Node3D)->void:
    ...
    var shoulder_l:MeshInstance3D=_sphere(accent_mat,0.19,Vector3(-0.52,1.67,0))
    var shoulder_r:MeshInstance3D=_sphere(accent_mat,0.19,Vector3(0.52,1.67,0))
    root.add_child(shoulder_l)
    root.add_child(shoulder_r)
    if class_id=="Archer" or class_id=="Ranger":
        for i in range(3):
            var arrow:MeshInstance3D=_cyl(gold,0.015,0.72,Vector3(-0.48+float(i)*0.045,1.55,0.24))
            root.add_child(arrow)
    elif class_id=="Mage":
        var gem:MeshInstance3D=_sphere(accent_mat,0.11,Vector3(0,2.60,-0.34))
        root.add_child(gem)
        ...
        var mask:MeshInstance3D=_box(_mat(Color("#252b34")),Vector3(0.46,0.13,0.05),Vector3(0,2.31,-0.36))
        root.add_child(mask)
