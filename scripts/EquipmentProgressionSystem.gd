...
static func normalize_item(item:Dictionary)->Dictionary:
    ...
    result["age_refine_success_bonus"]=max(0.0,float(result.get("age_refine_success_bonus",0.0)))
    return result
...
static func _apply_card(total:Dictionary,card_id:String)->void:
    ...
    total["hp"]+=int(data.get("hp",0)); total["def"]+=int(data.get("defense",0)); total["crit"]+=int(data.get("crit",0)); total["evasion"]+=int(data.get("evasion",0)); total["healing"]+=int(data.get("healing",0))
    for key in ["move_percent","fire_percent","wind_percent","dark_percent","ice_resist_percent","boss_damage_percent","all_rewards_percent","damage_percent","xp_percent","item_drop_percent","attack_speed_percent","magic_resist_percent","crit_damage_percent"]: total[key]+=float(data.get(key,0.0))
    total["poison_resist"]+=int(data.get("poison_resist",0)); total["power_bonus"]+=int(data.get("power",0))
...
static func total_stats(equipment:Dictionary)->Dictionary:
    ...
    for slot in equipment.keys():
        ...
        var rb:Dictionary=refined_bonus(item); total["atk"]+=int(rb["atk"]); total["matk"]+=int(rb["matk"]); total["def"]+=int(rb["def"])
        for card_id in item["cards"]: _apply_card(total,str(card_id))
    return total

static func refine_chance(current:int,age_refine_success_bonus:float=0.0)->float:
    var target:int=current+1
    if target<=1: return clampf(1.0+age_refine_success_bonus,0.0,0.99)
    var base:float=0.0
    if target<=5: base=0.90-float(target-2)*0.05
    elif target<=10: base=0.72-float(target-6)*0.08
    elif target<=15: base=0.38-float(target-11)*0.065
    return clampf(base+max(0.0,age_refine_success_bonus),0.0,0.99)
...
static func attempt_refine(item:Dictionary,roll:float)->Dictionary:
    ...
    var chance:float=refine_chance(current,float(normalized.get("age_refine_success_bonus",0.0)))
...
static func card_capacity(item:Dictionary)->int: return clamp(int(item.get("card_slots",0)),0,MAX_CARD_SLOTS)

static func _card_compatible(item:Dictionary,card_id:String)->bool:
    var card:Dictionary=_card_effect(card_id); var required:String=str(card.get("slot","")); if required=="": return true
    var type:String=str(item.get("type","")); var slot:String=str(item.get("slot",""))
