...
static func add_item(hero:Dictionary,item_name:String,amount:int=1)->bool:
	ensure_state(hero); var catalog:=ItemDatabase.all(); if amount<=0 or not catalog.has(item_name): return false
	...
	var inv:Dictionary=hero.get("inventory",{}); inv[item_name]=int(inv.get(item_name,0))+amount; hero["inventory"]=inv; hero["loot_stats"]["items"]+=amount
...
static func add_card(hero:Dictionary,card_name:String)->bool:
	ensure_state(hero); var catalog:=CardDatabase.all(); if card_name=="" or not catalog.has(card_name): return false
	if not bool(hero["loot_rules"].get("auto_pick_cards",true)) or not accept(hero,str(catalog[card_name].get("rarity","Common"))): return false
	if hero["cards"].has(card_name): return false
	hero["cards"].append(card_name); hero["loot_stats"]["cards"]+=1; return true
...
static func apply_bonus_loot(hero:Dictionary,monster:Dictionary,rng:RandomNumberGenerator)->Array[String]:
	ensure_state(hero); var gained:Array[String]=[]; var name:=str(monster.get("name","")); var card_name:=CardDatabase.for_monster(name)
	if card_name!="":
		var rarity:=str(CardDatabase.all()[card_name].get("rarity","Common")); if rng.randf()<CardDatabase.rarity_weight(rarity) and collect_drop(hero,card_name,name,rng): gained.append(card_name)
