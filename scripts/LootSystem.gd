...
static func add_card(hero:Dictionary,card_name:String)->bool:
	ensure_state(hero)
	if not bool(hero["loot_rules"].get("auto_pick_cards",true)) or not accept(hero,str(CardDatabase.all().get(card_name,{}).get("rarity","Common"))): return false
...
static func collect_drop(hero:Dictionary,name:String,monster_name:String,rng:RandomNumberGenerator)->bool:
	...
	if CardDatabase.all().has(name):
		...
		queue_ground(hero,name,kind,monster_name); return false
...
static func apply_bonus_loot(hero:Dictionary,monster:Dictionary,rng:RandomNumberGenerator)->Array[String]:
	...
	if card_name!="":
		...
		if rng.randf()<CardDatabase.rarity_weight(rarity):
			if collect_drop(hero,card_name,name,rng): gained.append(card_name)
...
static func apply_mvp_loot(hero:Dictionary,monster:Dictionary,rng:RandomNumberGenerator)->Array[String]:
	...
	if card_name!="" and collect_drop(hero,card_name,str(monster.get("name","MVP")),rng): gained.append(card_name)
	for item_name in monster.get("mvp_loot",[]):
		if rng.randf()<0.78 and collect_drop(hero,str(item_name),str(monster.get("name","MVP")),rng): gained.append(str(item_name))
	if rng.randf()<0.35 and collect_drop(hero,"MVP Bounty Token",str(monster.get("name","MVP")),rng): gained.append("MVP Bounty Token")
	if collect_drop(hero,"MVP Treasure Chest",str(monster.get("name","MVP")),rng): gained.append("MVP Treasure Chest")
	return gained

static func on_monster_defeated(hero:Dictionary,monster:Dictionary,rng:RandomNumberGenerator)->Array[String]:
	ensure_state(hero)
	var gained:Array[String]=[]
	var drops:=WorldSystem.roll_drops(str(monster.get("name","")),rng)
	for drop in drops:
