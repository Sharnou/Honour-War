...
var combat_vfx:CombatVFXClass

func _ready()->void:
	game=get_parent()
	combat_vfx=CombatVFXClass.new()
...
func _input(event:InputEvent)->void:
	...
	if event is InputEventKey and event.pressed and not event.echo:
		if event.keycode>=KEY_1 and event.keycode<=KEY_8:
			...
			var ids:=SkillSystemClass.all_skills(str(hero.get("class","Warrior")))
			if index<ids.size():
...
func refresh()->void:
	...
	skill_buttons.clear()
	var skills:=SkillSystemClass.all_skills(str(hero.get("class","Warrior")))
	var current_tier:=0
	var slot:=0
	for skill in skills:
		...
		var rank:=SkillSystemClass.skill_level(hero,str(skill["id"]))
...
func update_detail()->void:
	...
	if not hero_value is Dictionary: return
	var hero:Dictionary=hero_value
	var skills:=SkillSystemClass.skill_map(str(hero.get("class","Warrior")))
	if not skills.has(selected_skill_id): return
	var skill:Dictionary=skills[selected_skill_id]
	var level:=SkillSystemClass.skill_level(hero,selected_skill_id)
	var req_text:="None"
	if skill["requires"].size()>0: req_text=", ".join(skill["requires"])
	var stats:=SkillSystemClass.combat_stats(hero)
	var effect:=SkillSystemClass.effect_text(hero,selected_skill_id)
	detail_label.text="%s\n\nType: %s\nTier: %d\nRank: %d / %d\nRequired Hero Level: %d\nUpgrade Cost: %d SP\nSkill SP Cost: %d\nCooldown: %.1fs\nPower: %d\nPrerequisites: %s\n\nEffect: %s\n\nLIVE PASSIVE BONUSES\nPower +%d  Crit +%d%%\nDefense +%d  Healing +%d\nRefine +%d\n\n%s" % [skill["name"],str(skill["kind"]).to_upper(),skill["tier"],level,skill["max_level"],skill["required_level"],skill["cost"],SkillSystemClass.sp_cost(hero,selected_skill_id),skill["cooldown"],SkillSystemClass.power(hero,selected_skill_id),req_text,effect,stats["power_bonus"],stats["crit_bonus"],stats["defense_bonus"],stats["healing_bonus"],stats["refine_bonus"],skill["description"]]

func learn_skill(skill_id:String)->void:
	var hero:Dictionary=game.get("hero")
	if SkillSystemClass.learn(hero,skill_id):
		selected_skill_id=skill_id
		game.call("log_message","Skill upgraded: %s Lv.%d. Passive effects are now active." % [SkillSystemClass.skill_map(str(hero["class"]))[skill_id]["name"],SkillSystemClass.skill_level(hero,skill_id)])
		game.call("save_game")
	else:
...
func use_skill(skill_id:String)->void:
	...
	var skill_map:=SkillSystemClass.skill_map(str(hero.get("class","Warrior")))
	...
	if target==null and not targetless_heal:
		...
		refresh()
		return
	var result:=SkillSystemClass.use(hero,skill_id,Time.get_ticks_msec()/1000.0)
	if not result["ok"]:
	...
	if target==null:
		...
		target["hp"]-=damage
		var critical:=int(result["power"])>=SkillSystemClass.power(hero,skill_id)*2
