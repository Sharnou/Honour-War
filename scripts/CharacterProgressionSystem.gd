...
static func _save(hero:Dictionary)->void:
	Save.save_game(hero)
...
static func allocate(hero:Dictionary,stat:String,amount:int=1)->bool:
	...
	var spend:int=min(amount,STAT_CAP-current,int(hero.get("stat_points",0)))
...
static func stats(hero:Dictionary)->Dictionary:
	...
	var age:Dictionary=Age.strength_bonus(hero)
	...
	var hit:int=level+int(s["dex"])*2+int(e["hit"])+int(age["hit"])
	var flee:int=level+int(s["agi"])*2+int(e["flee"])+int(age["flee"])
...
static func level_up(hero:Dictionary,new_level:int)->int:
	...
	var live:Dictionary=stats(hero)
	hero["hp"]=min(int(live["max_hp"]),max(1,int(hero.get("hp",live["max_hp"]))))
	hero["sp"]=min(int(live["max_sp"]),max(0,int(hero.get("sp",live["max_sp"]))))
	_save(hero)
	return gained

static func xp_to_next(level:int)->int:
	var l:int=clamp(level,1,MAX_LEVEL)
	return 100+int(pow(float(l),1.55)*35.0)

static func xp_progress(hero:Dictionary)->Dictionary:
	ensure_state(hero)
	var level:int=int(hero["level"])
	if level>=MAX_LEVEL: return {"xp":0,"next":0,"ratio":1.0,"level":level}
	var next:int=xp_to_next(level)
	var current:int=clamp(int(hero.get("xp",0)),0,next)
	return {"xp":current,"next":next,"ratio":float(current)/float(max(1,next)),"level":level}

static func grant_xp(hero:Dictionary,xp:int)->Dictionary:
	ensure_state(hero)
	var current:int=int(hero.get("xp",0))+max(0,xp)
	var gained_levels:int=0
	var gained_points:int=0
	while int(hero["level"])<MAX_LEVEL and current>=xp_to_next(int(hero["level"])):
		current-=xp_to_next(int(hero["level"]))
		var points:int=level_up(hero,int(hero["level"])+1)
		gained_points+=points
		gained_levels+=1
	hero["xp"]=current
	_save(hero)
	return {"xp":current,"levels":gained_levels,"level":int(hero["level"]),"stat_points":int(hero["stat_points"]),"gained_points":gained_points}

static func combat_power(hero:Dictionary)->int:
	var s:Dictionary=stats(hero)
	return int(s["atk"]*1.0+s["matk"]*1.0+s["def"]*0.7+s["mdef"]*0.7+s["max_hp"]*0.08+s["hit"]*0.5+s["flee"]*0.5+s["crit"]*2.0)
