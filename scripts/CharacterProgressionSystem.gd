...
static func allocate(hero:Dictionary,stat:String,amount:int=1)->bool:
	...
	hero["stats"][stat]=current+spend; hero["stat_points"]-=spend
	return true
...
static func reset_stats(hero:Dictionary)->int:
	...
	for stat in STAT_NAMES:
		refund+=max(0,int(hero["stats"].get(stat,1))-1); hero["stats"][stat]=1
...
static func equipment_bonus(hero:Dictionary)->Dictionary:
	...
	for slot in hero["equipment"].keys():
		...
		for key in result.keys(): result[key]+=int(item.get(key,0))
		var refine:int=int(hero["equipment_refine"].get(slot,0))
		result["atk"]+=refine*int(item.get("refine_atk",2)); result["matk"]+=refine*int(item.get("refine_matk",2)); result["def"]+=refine*int(item.get("refine_def",1))
	return result
...
static func stats(hero:Dictionary)->Dictionary:
	...
	return {"max_hp":hp,"max_sp":sp,"atk":atk,"matk":matk,"def":defense,"mdef":mdef,"hit":level+int(s["dex"])*2+int(e["hit"])+int(age["hit"]),"flee":level+int(s["agi"])*2+int(e["flee"])+int(age["flee"]),"crit":int(s["luk"])/2+int(e["crit"])+int(age["crit"]),"healing":int(age["healing"])}
...
static func level_up(hero:Dictionary,new_level:int)->int:
	...
	hero["level"]=target; hero["stat_points"]+=gained
	return gained
