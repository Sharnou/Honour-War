...
func _use_item(id:String)->void:
    ...
    if value is Dictionary:
        ...
        if bool(result.get("ok",false)):
            ...
            _label("Action failed: "+str(result.get("reason","unavailable")))
    _render_mode()
...
func _equipment(hero:Dictionary)->void:
    ...
    for slot in INVENTORY.EQUIPMENT_SLOTS:
        ...
        body.add_child(b)
    _label("Click an occupied slot to unequip. Equipped items remain part of the live combat calculation.")
...
func _refine_slot(slot:String)->void:
    ...
    if value is Dictionary:
        ...
        if bool(result.get("ok",false)):
            SAVE.save_game(value)
        if status!=null and not bool(result.get("ok",false)):
...
func _objectives(hero:Dictionary)->void:
    ...
    _objective_row("Build a city base",hero.get("city_building",{}) is Dictionary and not (hero.get("city_building",{}) as Dictionary).is_empty())
...
func _system(hero:Dictionary)->void:
    ...
    body.add_child(full)
    _label("The restore button is for normal town/rest use during testing; it does not modify equipment or progression.")
