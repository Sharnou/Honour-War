...
## Compatibility-safe HD presentation layer.
## It keeps gameplay geometry in Godot but upgrades the visible hero and town
## with complete silhouettes, readable facial features, layered clothing,
## authored-looking buildings, roads, lamps, signs and foliage.

const CENTER := Vector3(12.925,0.0,12.65)
var root:Node3D
var hero_built:bool=false
var environment_built:bool=false
...
func _build()->void:
    ...
    if actors:
        ...
        if hero:
            _upgrade_hero(hero,str((game.get("legacy") as Node2D).get("hero",{}).get("class","Warrior")))
            hero_built=true
...
func _build_ground()->void:
    var ground:=_mat(Color("#4D6946"),0.94)
    _box(root,Vector3(74.0,0.35,48.0),CENTER+Vector3(0.0,-0.20,0.0),ground)
    var stone:=_mat(Color("#8C806C"),0.88)
    var road:=_mat(Color("#6B5646"),0.96)
    _box(root,Vector3(11.0,0.12,46.0),CENTER+Vector3(0,-0.02,0),road)
    _box(root,Vector3(72.0,0.12,7.2),CENTER+Vector3(0,-0.02,1.0),road)
    _box(root,Vector3(38.0,0.10,5.0),CENTER+Vector3(0,0.02,-10.0),stone)
    for x in range(-4,5):
        _box(root,Vector3(0.28,0.03,9.4),CENTER+Vector3(float(x)*1.95,0.055,-10.0),_mat(Color("#B4A48A"),1.0))
    var plaza:=_cyl(root,6.4,0.22,CENTER+Vector3(0,0.12,5.5),stone,64)
    var water:=_cyl(root,3.1,0.10,CENTER+Vector3(0,0.28,5.5),_mat(Color("#5E9EB2"),0.22),64)
    water.material_override.next_pass=_mat(Color("#5E9EB2"),0.18)

func _build_town()->void:
    var wall_mats:Array[StandardMaterial3D]=[_mat(Color("#B98261")),_mat(Color("#C6A373")),_mat(Color("#829B73")),_mat(Color("#81769A"))]
    var roof_mats:Array[StandardMaterial3D]=[_mat(Color("#5B2830"),0.72),_mat(Color("#3D455E"),0.72),_mat(Color("#6A4B35"),0.75),_mat(Color("#314C3E"),0.75)]
    ...
    for i in points.size(): _house(points[i],wall_mats[i%4],roof_mats[i%4],i)
    _lamp_posts()
    _trees()
    _build_gate(CENTER+Vector3(0,0,-17))
...
func _house(pos:Vector3,wall:Material,roof:Material,index:int)->void:
    ...
    var roof_mesh:=CylinderMesh.new(); roof_mesh.top_radius=0.0; roof_mesh.bottom_radius=4.5; roof_mesh.height=2.5; roof_mesh.radial_segments=6
    var r:=MeshInstance3D.new(); r.mesh=roof_mesh; r.position=pos+Vector3(0,5.15,0); r.material_override=roof; h.add_child(r)
    var trim:=_mat(Color("#E2CC9C"),0.55)
    _box(h,Vector3(5.7,0.18,0.16),pos+Vector3(0,3.72,2.73),trim)
    _box(h,Vector3(1.05,1.95,0.12),pos+Vector3(0,0.98,2.75),_mat(Color("#3B2922"),0.8))
    for sx in (-1.65,1.65):
        _box(h,Vector3(1.15,1.05,0.10),pos+Vector3(sx,2.25,2.75),_mat(Color("#88C5D4"),0.16))
        _box(h,Vector3(0.10,1.05,0.12),pos+Vector3(sx,2.25,2.82),trim)
        _box(h,Vector3(1.15,0.10,0.12),pos+Vector3(sx,2.25,2.82),trim)
    var sign:=_box(h,Vector3(1.5,0.55,0.10),pos+Vector3(0,3.55,2.82),_mat(Color("#6E4D2F"),0.75))
    sign.rotation_degrees.z=2.0+float(index%3)*1.5
    var banner:=_box(h,Vector3(0.85,1.35,0.05),pos+Vector3(-2.45,2.8,2.80),roof)
    banner.rotation_degrees.z=-7.0

func _lamp_posts()->void:
    var dark:=_mat(Color("#232730"),0.62,0.5); var gold:=_mat(Color("#D9B75A"),0.35,0.7)
    for x in (-19.0,-7.0,7.0,19.0):
        for z in (-1.0,9.5):
            var p:=CENTER+Vector3(x,0,z); _cyl(root,0.09,2.9,p+Vector3(0,1.45,0),dark,24); _sphere(root,0.16,p+Vector3(0,3.0,0),gold,Vector3(1.0,1.0,0.75))
...
func _trees()->void:
    ...
    for i in pts.size():
        var p:Vector3=pts[i]; _cyl(root,0.32,2.8,p+Vector3(0,1.4,0),trunk,24); _sphere(root,1.55,p+Vector3(0,3.3,0),leaf,Vector3(1.2,1.0,1.15)); _sphere(root,1.05,p+Vector3(0.9,3.8,0.2),leaf,Vector3(1.0,0.85,1.0))

func _build_gate(pos:Vector3)->void:
    var stone:=_mat(Color("#6B6B73"),0.84); var banner:=_mat(Color("#A6424E"),0.7)
...
func _upgrade_hero(hero:Node3D,class_id:String)->void:
    ...
    var skin:=_mat(Color("#D69B78"),0.62); var cloth:=_mat(accent,0.52); var dark:=_mat(Color("#20252D"),0.7); var metal:=_mat(Color("#C2CBD4"),0.22,0.75); var gold:=_mat(Color("#E9C66A"),0.25,0.8); var hair:=_mat(Color("#2A242C"),0.50)
    ...
    _box(hero,Vector3(0.12,0.25,0.18),Vector3(0.34,2.34,0.02),hair)
    var eye:=_mat(Color("#24314A"),0.3)
    for x in (-0.12,0.12):
        _box(hero,Vector3(0.08,0.05,0.025),Vector3(x,2.36,-0.34),eye)
        _box(hero,Vector3(0.055,0.025,0.025),Vector3(x,2.28,-0.355),_mat(Color("#F5F3EA"),0.4))
