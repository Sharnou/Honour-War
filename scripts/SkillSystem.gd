...
static func ensure_state(hero:Dictionary)->void:
	...
	if not hero.has("skill_points_level"): hero["skill_points_level"]=int(hero.get("level",1))
	var current_level:=int(hero.get("level",1))
	var recorded_level:=int(hero.get("skill_points_level",current_level))
...
static func passive_healing_bonus(hero:Dictionary)->int:
	if str(hero.get("class","Warrior"))=="Acolyte": return skill_level(hero,"aco_divine_grace")*4+skill_level(hero,"aco_blessing")*6
	return 0

static func passive_refine_bonus(hero:Dictionary)->int:
	if str(hero.get("class","Warrior"))=="Merchant": return skill_level(hero,"mer_master_crafter")
	return 0
...
static func is_ready(hero:Dictionary,skill_id:String,now:float)->bool:
	return float(hero.get("skill_cooldowns",{}).get(skill_id,0.0))<=now
...
static func effect_text(hero:Dictionary,skill_id:String)->String:
	...
	match skill_id:
		"war_guardian_roar": return "Defense strengthened for the hero and pet."
		"war_earthbreaker": return "Heavy strike that breaks enemy defense for follow-up attacks."
		"war_emperors_judgment": return "Royal execution strike that strongly breaks enemy defense."
		"war_immortal_arsenal": return "Ultimate strike that shatters enemy defense for a long duration."
		"mage_frost_prison": return "Target is rooted and slowed by the frost field."
		"mage_comet", "mage_meteor_surge": return "Burning magic that applies damage over time."
		"mage_void_lance", "mage_arcane_overload": return "Void magic that breaks enemy defense for follow-up attacks."
		"arch_trap": return "Lightning trap roots and slows the target."
		"arch_skybreaker": return "Piercing shot that breaks enemy defense."
		"thief_smoke": return "Hero gains a short evasion window."
		"aco_sanctuary": return "Creates a healing sanctuary around the hero."
		"aco_seraphic_light", "aco_heaven_gate": return "Deals holy damage while restoring the hero and pet."
		"mer_fortify": return "Defense strengthened and refinement efficiency improved."
		"mer_magma_forge": return "Superheated strike that applies burning damage over time."
		"mer_arsenal_overlord": return "Hero and pet equipment receive a temporary power surge."
		_: return "Direct combat damage."
