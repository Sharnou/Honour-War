...
func _build() -> void:
    ...
    status_label.text = "1-6: pet command states"
...
func _refresh_skills(pet:Dictionary, species:String) -> void:
    ...
    for skill in PetSkillSystem.all_skills(species):
        signature += ":" + str(PetSkillSystem.skill_level(pet, str(skill["id"])))
    ...
    for skill in PetSkillSystem.all_skills(species):
        ...
        if str(skill.get("kind", "")) == "passive":
            continue
        if PetSkillSystem.skill_level(pet, id) <= 0:
            continue
        var button := Button.new()
...
func _use_skill(skill_id:String) -> void:
    ...
    if runtime and runtime.has_method("request_skill"):
        ...
        if bool(result.get("ok", false)):
            ...
            _set_status("Skill unavailable: " + str(result.get("reason", "rejected")))
    else:
        _set_status("Pet skill runtime is not ready.")
