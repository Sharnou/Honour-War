...
# Procedural HD combat presentation. No external art assets required.
# Consumes HDCombatFeedback events and keeps the effect count bounded.
...
func _ready() -> void:
	...
	if feedback == null:
		return
	if feedback.has_signal("damage_number_requested"):
		feedback.damage_number_requested.connect(_on_damage)
	if feedback.has_signal("telegraph_requested"):
		feedback.telegraph_requested.connect(_on_telegraph)
	if feedback.has_signal("skill_effect_requested"):
...
func _on_damage(amount:int, world_position:Vector3, critical:bool) -> void:
	...
	tween.tween_property(label, "modulate:a", 0.0, damage_lifetime).set_delay(0.18)
...
func _on_skill(effect_id:String, world_position:Vector3) -> void:
	...
	tween.tween_property(impact, "modulate:a", 0.0, 0.28)
	tween.set_parallel(false)
