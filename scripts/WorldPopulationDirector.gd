...
func _sync_population()->void:
	...
	if legacy.has_method("log_message"):
		legacy.call("log_message","Map populated: %d combat creatures are active." % MIN_MONSTERS)
...
func _replenish()->void:
	...
	for item in monsters:
		if item is Dictionary and int((item as Dictionary).get("hp",0))>0:
			continue
...
func _spawn_one()->void:
	...
	for attempt in 8:
		...
		if pos.distance_to(hero_pos)>150.0 or monsters.is_empty(): break
	monsters.append({"name":family,"level":level,"pos":pos,"hp":stats["max_hp"],"max":stats["max_hp"],"attack":stats["attack"],"defense":stats["defense"],"exp":stats["exp"],"zmin":stats["zeny_min"],"zmax":stats["zeny_max"]})
