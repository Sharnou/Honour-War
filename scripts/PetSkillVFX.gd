...
func _falcon(skill_id:String, position:Vector3, radius:float) -> void:
    if skill_id.find("mark") >= 0 or skill_id.find("eye") >= 0:
        _target_ring(position, 0.8, 1.0)
    elif skill_id.find("storm") >= 0 or skill_id.find("tempest") >= 0 or skill_id.find("requiem") >= 0:
        _vortex(position, radius, 1.15)
    else:
        _slash(position, 0.9, 2.8)

func _wolf(skill_id:String, position:Vector3, radius:float) -> void:
    if skill_id.find("howl") >= 0:
        _shockwave(position, radius, 1.0)
    elif skill_id.find("fury") >= 0 or skill_id.find("alpha") >= 0:
        _shockwave(position, radius * 0.8, 1.5)
        _slash(position, 1.1, 3.4)
    else:
        _slash(position, 0.75, 4.0)

func _dragon(skill_id:String, position:Vector3, radius:float) -> void:
    if skill_id.find("meteor") >= 0 or skill_id.find("apocalypse") >= 0 or skill_id.find("eternity") >= 0:
        _meteor(position, radius)
    elif skill_id.find("wing") >= 0:
        _arc(position, radius, 1.8)
    else:
        _burst(position, radius * 0.7, 1.3)

func _wolf_cub(skill_id:String, position:Vector3, radius:float) -> void:
    if skill_id.find("guardian") >= 0 or skill_id.find("bond") >= 0 or skill_id.find("legend") >= 0:
        _shield(position, radius)
    elif skill_id.find("howl") >= 0:
        _shockwave(position, radius * 0.75, 0.8)
    else:
        _slash(position, 0.55, 2.0)
