...
func _ready() -> void:
	...
	log_message("Welcome to Honour War. Your adventure is saved automatically.")
	queue_redraw()
...
func ensure_runtime_state() -> void:
	...
	hero["age"] = 18 + int(float(hero.get("online_days", 0.0)) / GameData.AGE_DAYS_PER_YEAR)
...
func build_ui() -> void:
	...
	side_box.add_child(apply)
	var rest := Button.new()
	rest.text = "Rest (+1 simulated online day)"
	rest.pressed.connect(rest_hero)
	side_box.add_child(rest)
	var city := Button.new()
	city.text = "Build Prontera (+1 building)"
	city.pressed.connect(build_city)
	side_box.add_child(city)
	var save := Button.new()
	save.text = "Save Now"
	...
	var help := Label.new()
	help.text = "WASD / Arrows: move   SPACE: attack   R: refine   Q: quest   C: craft   B: buy material"
	...
	var class_index := CLASS_NAMES().find(str(hero.get("class", "Warrior")))
	class_box.select(max(0, class_index))
	update_ui()
...
func apply_hero() -> void:
	hero["name"] = name_edit.text.strip_edges() if name_edit.text.strip_edges() != "" else str(hero.get("name", "Aldric"))
...
func rest_hero() -> void:
	...
	hero["age"] = 18 + int(float(hero["online_days"]) / GameData.AGE_DAYS_PER_YEAR)
	hero["hp"] = hero["max_hp"]
	log_message("A full simulated online day passed. Age: %d." % hero["age"])
	save_game()
	update_ui()
...
func build_city() -> void:
	...
	var building_names := ["Blacksmith", "Market", "Barracks", "Magic Tower"]
	var built := false
...
func _process(delta:float) -> void:
	...
	hero["pos_x"] = clamp(float(hero.get("pos_x", 270.0)) + movement.x * 180.0 * delta, 365.0, 1110.0)
	hero["pos_y"] = clamp(float(hero.get("pos_y", 330.0)) + movement.y * 180.0 * delta, 150.0, 420.0)
...
func spawn_monster() -> void:
	...
	var monster_level := WorldSystem.monster_level_for_zone(zone_level, rng.randi_range(0, families.size() - 1))
	var stats:Dictionary = WorldSystem.monster_stats(monster_level)
	monsters.append({"name":family, "level":monster_level, "pos":Vector2(rng.randf_range(410.0,1080.0),rng.randf_range(175.0,405.0)), "hp":stats["max_hp"], "max":stats["max_hp"], "attack":stats["attack"], "exp":stats["exp"], "zeny_min":stats["zeny_min"], "zeny_max":stats["zeny_max"]})

func nearest_monster():
	var best = null
	var dist := 99999.0
	var hero_pos := Vector2(float(hero["pos_x"]), float(hero["pos_y"]))
	for monster in monsters:
		var d:float = hero_pos.distance_to(monster["pos"])
		if d < dist:
			dist = d
			best = monster
	return best if dist < 90.0 else null
...
func refine() -> void:
	var refine_level := int(hero.get("refine", 0))
	var material := "Phracon" if refine_level < 4 else ("Emveretarcon" if refine_level < 8 else "Oridecon")
	if refine_level >= 15:
		log_message("Equipment has reached the prototype refinement cap +15.")
		return
	if int(hero["materials"].get(material, 0)) <= 0:
		log_message("You need %s to refine." % material)
		return
	hero["materials"][material] -= 1
	var chance := refine_chance()
	if rng.randf() * 100.0 < chance:
		hero["refine"] = refine_level + 1
		hero["equipment"]["weapon"] = CLASSES[hero["class"]]["weapon"] + " +" + str(hero["refine"])
		log_message("Refinement succeeded! Weapon is now +%d." % hero["refine"])
...
func quest() -> void:
	...
	add_exp(50)
	log_message("Quest completed: %s. Rewards: EXP, Zeny, Phracon." % quest_id)
	save_game()
	update_ui()
...
func craft() -> void:
	...
	hero["zeny"] -= 80
	var item_name := CLASSES[hero["class"]]["weapon"] + " Core"
	hero["inventory"][item_name] = int(hero["inventory"].get(item_name, 0)) + 1
	log_message("Crafted %s. It was added to inventory." % item_name)
	save_game()
	update_ui()
...
func update_ui() -> void:
	...
	var city:Dictionary = hero["city_building"].get("Prontera", CitySystem.new_city())
	var building_count := int(city.get("buildings", {}).size())
	profile.text = "Name: %s\nClass: %s\nLevel: %d / %d\nEXP: %d / %d\nAge: %d years\nOnline: %.4f days\nHP: %d/%d\nZeny: %d\n\nMaterials\nPhracon: %d\nEmveretarcon: %d\nOridecon: %d\n\nAge skill bonus: +%d%%\nRefine chance: %d%%\nPrice discount: -%d%%\nWeapon: %s\nCards: %d\nInventory entries: %d\nProntera Lv.%d / Buildings: %d" % [hero["name"], GameData.class_title(hero), hero["level"], GameData.MAX_HERO_LEVEL, hero["exp"], GameData.exp_to_next(int(hero["level"])), hero["age"], hero["online_days"], hero["hp"], hero["max_hp"], hero["zeny"], mats["Phracon"], mats["Emveretarcon"], mats["Oridecon"], age_bonus(), refine_chance(), int(WorldSystem.age_discount(int(hero["age"])) * 100.0), hero["equipment"]["weapon"], hero["cards"].size(), hero["inventory"].size(), city.get("level", 1), building_count]
...
func _draw() -> void:
	...
	var hero_pos := Vector2(float(hero["pos_x"]), float(hero["pos_y"]))
	draw_circle(hero_pos, 18, data["color"])
	draw_string(ThemeDB.fallback_font, Vector2(hero_pos.x - 28, hero_pos.y - 28), str(hero["name"]), HORIZONTAL_ALIGNMENT_LEFT, -1, 13, Color.WHITE)
	for monster in monsters:
