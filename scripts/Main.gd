...
func ensure_pet_state()->void:
	...
	p["max_hp"]=int(p.get("max_hp",60))
	p["hp"]=clamp(int(p.get("hp",p["max_hp"])),0,int(p["max_hp"]))
	p["max_sp"]=int(p.get("max_sp",30))
	p["sp"]=clamp(int(p.get("sp",p["max_sp"])),0,int(p["max_sp"]))
	p["skills"]=p.get("skills",[d["skill"]])
	p["skill_level"]=max(1,int(p.get("skill_level",1)))
	p["skill_points"]=int(p.get("skill_points",0))
	p["skill_uses"]=int(p.get("skill_uses",0))
	p["refine"]=clamp(int(p.get("refine",0)),0,15)
	p["inventory"]=p.get("inventory",{})
	p["materials"]=p.get("materials",{"Phracon":3,"Emveretarcon":1,"Oridecon":0})
	p["equipment"]=p.get("equipment",{"weapon":d["species"]+" Claw","armor":"Pet Guard Harness"})
...
func apply_hero()->void:
	...
	ensure_pet_state(); update_pet_visual()
...
func rest_hero()->void:
	...
	update_age(); hero["hp"]=hero["max_hp"]; hero["pet"]["hp"]=hero["pet"]["max_hp"]
...
func _process(delta:float)->void:
	...
	hero["pos_x"]=clamp(float(hero["pos_x"])+move.x*180.0*delta,365.0,1107.0)
	hero["pos_y"]=clamp(float(hero["pos_y"])+move.y*180.0*delta,120.0,420.0)
	update_pet_visual()
...
func fast_travel(map_id:int,x:int,y:int)->void:
	...
	hero["map_id"]=map_id; hero["pos_x"]=365.0+float(x); hero["pos_y"]=120.0+float(y); monsters.clear()
	for i in 4: spawn_monster()
	log_message("Fast transmission to %s at X:%d Y:%d. Pet follows automatically." % [TeleportSystem.map_name(map_id),x,y]); save_game(); update_ui()
...
func attack()->void:
	...
	var damage:=skill_power()+rng.randi_range(0,9); m["hp"]-=damage; log_message("%s dealt %d damage to Lv.%d %s." % [CLASSES[hero["class"]]["skill"],damage,m["level"],m["name"]]); pet_attack_target(m); if m["hp"]<=0: defeat_monster(m); save_game(); update_ui()
...
func pet_attack_target(m:Dictionary)->void:
	...
	if used_skill:
		...
		if str(pet["role"])=="Healer": hero["hp"]=min(int(hero["max_hp"]),int(hero["hp"])+PetSystem.heal_power(pet))
...
func defeat_monster(m:Dictionary)->void:
	hero["kills"]=int(hero.get("kills",0))+1; hero["pet"]["kills"]=int(hero["pet"].get("kills",0))+1; var z:=rng.randi_range(int(m["zmin"]),int(m["zmax"])); hero["zeny"]+=z; add_exp(int(m["exp"])); var pet_exp:=max(1,int(m["exp"])*2/3); var pet_leveled:=PetSystem.add_exp(hero["pet"],pet_exp); if pet_leveled: log_message("%s reached Pet Lv.%d and gained a skill point." % [hero["pet"]["name"],hero["pet"]["level"]])
	...
	log_message("Defeated Lv.%d %s! Hero +%d EXP, Pet +%d EXP, +%d Zeny." % [m["level"],m["name"],m["exp"],pet_exp,z]); monsters.erase(m)
...
func refine()->void:
	...
	if int(hero["materials"].get(material,0))<=0: log_message("You need %s." % material); return
...
func refine_pet()->void:
	...
	if rng.randf()*100.0<chance: pet["refine"]=level+1; pet["equipment"]["weapon"]=str(pet["species"])+" Claw +"+str(pet["refine"]); log_message("%s refinement succeeded: +%d." % [pet["name"],pet["refine"]])
...
func update_ui()->void:
	...
	ensure_pet_state(); var m:Dictionary=hero["materials"]; var p:Dictionary=hero["pet"]; var city:Dictionary=hero["city_building"].get("Prontera",CitySystem.new_city()); var map_name:=TeleportSystem.map_name(int(hero["map_id"])); var map_x:=int(hero["pos_x"]-365.0); var map_y:=int(hero["pos_y"]-120.0)
	...
	status.text="%s | MAP %d: %s | X:%d Y:%d | Hero Power:%d | Pet:%s [%s] Power:%d | Monsters:%d | Kills:%d" % ["Dungeon" if TeleportSystem.is_dungeon(int(hero["map_id"])) else "Town",int(hero["map_id"]),map_name,map_x,map_y,skill_power(),p["name"],p["role"],PetSystem.power(p),monsters.size(),hero.get("kills",0)]
	log_label.text="\n".join(logs)
