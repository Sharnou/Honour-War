...
## danger rating and drop families. The same data can drive UI and VFX.
static func details(monster:Dictionary)->Dictionary:
	...
	var data:Dictionary={
		"name":name,"level":int(monster.get("level",1)),"role":"Melee",
		"element":"Neutral","status":"None","status_chance":0.0,
		"poison_resist":0.0,"danger":2,"weakness":"Neutral","description":"Hostile creature."
	}
	if lower.contains("poring"):
		data["role"]="Melee"; data["element"]="Water"; data["danger"]=1; data["description"]="Small roaming creature with quick recovery."
	elif lower.contains("goblin"):
		data["role"]="Melee"; data["element"]="Earth"; data["status"]="Bleed"; data["status_chance"]=0.10; data["danger"]=2
		...
		data["role"]="Melee"; data["element"]="Undead"; data["status"]="Bleed"; data["status_chance"]=0.08; data["poison_resist"]=1.0; data["danger"]=3
	elif lower.contains("zombie"):
		data["role"]="Melee"; data["element"]="Undead"; data["status"]="Poison"; data["status_chance"]=0.22; data["poison_resist"]=0.65; data["danger"]=4
		...
		data["role"]="Executioner"; data["element"]="Dark"; data["status"]="Bleed + Fear"; data["status_chance"]=0.45; data["poison_resist"]=0.20; data["danger"]=12
		data["weakness"]="Holy"; data["description"]="A cursed armored executioner drenched in ancient blood."
	if mvp: data["danger"]=max(int(data["danger"]),15); data["description"]=str(data["description"])+" MVP-class threat."
	return data
...
static func apply_poison(monster:Dictionary,source_damage:int,duration:float=6.0)->bool:
	...
	monster["poison_until"]=max(float(monster.get("poison_until",0.0)),Time.get_ticks_msec()/1000.0+duration*effective)
	monster["poison_damage"]=max(1,int(float(source_damage)*0.25*effective))
	monster["poison_source"]="Hero/Pet"
	monster["poison_tick"]=Time.get_ticks_msec()/1000.0+1.0
	return true
