...
func _draw_pet(hero:Dictionary,pos:Vector2)->void:
	...
	if species=="Royal Falcon":
		draw_ellipse_custom(p,Vector2(10,15),c)
		...
		draw_ellipse_custom(p,Vector2(20,10),c)
...
func _draw_monsters(monsters:Variant)->void:
	...
	for monster in monsters:
		...
		if elite:
			...
			draw_arc(p,34*s+pulse,clock,clock+5.0,42,Color("#f5c85b"),3.0)
...
func _draw_wolf(p:Vector2,c:Color,s:float)->void:
	draw_ellipse_custom(p,Vector2(21,12)*s,c)
...
func _draw_dragon(p:Vector2,c:Color,s:float)->void:
	draw_ellipse_custom(p,Vector2(27,15)*s,c)
...
func _draw_mantis(p:Vector2,c:Color,s:float)->void:
	draw_ellipse_custom(p,Vector2(12,22)*s,c)
...
func _draw_shadow(center:Vector2,width:float,height:float)->void:
	draw_ellipse_custom(center,Vector2(width,height),Color(0.0,0.0,0.0,0.32))
