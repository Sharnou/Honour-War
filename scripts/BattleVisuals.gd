...
func _process(delta:float)->void:
	...
	var pos:=Vector2(float(hero.get("pos_x",0.0)),float(hero.get("pos_y",0.0)))
	var speed:=last_hero_pos.distance_to(pos)/max(delta,0.001)
	...
	if monsters is Array:
		for monster in monsters:
			...
			var old:=int(monster_seen.get(id,hp))
			if hp<old: monster_hit[id]=0.24
			monster_seen[id]=hp
	for id in monster_hit.keys(): monster_hit[id]=max(0.0,float(monster_hit[id])-delta)
	queue_redraw()

func trigger_hero_attack()->void: hero_action=0.22
func trigger_pet_attack()->void: pet_action=0.24
...
func _draw_hero(hero:Dictionary,pos:Vector2)->void:
	...
	var c:=Color(str(CLASS_COLORS.get(class_id,"#ffffff")))
	var breathe:=sin(clock*3.0)*1.5
	var stride:=sin(clock*10.0)*3.0*hero_motion
	var recoil:=-9.0 if hero_action>0.0 else 0.0
	...
	draw_line(p+Vector2(6,14),p+Vector2(6-stride,25),Color("#242b3a"),5.0)
	var hand:=p+Vector2(10,-2)
	var end:=hand+Vector2(25,-10)
	...
	for i in range(3):
		var sp:=p+Vector2(cos(clock*2+i*2.1)*23,sin(clock*2+i*2.1)*16)
...
func _draw_pet(hero:Dictionary,pos:Vector2)->void:
	...
	var p:=pos+Vector2(14 if pet_action>0 else 0,sin(clock*5)*3)
...
func _draw_monsters(monsters)->void:
	...
	for monster in monsters:
		...
		var c:=Color(str(MONSTER_COLORS.get(name,"#d45c5c")))
		var id:=str(monster.get("visual_id",""))
		var hit:=float(monster_hit.get(id,0.0))
		var recoil:=(-8.0 if int(monster.get("level",1))%2==0 else 8.0)*min(1.0,hit*5.0)
		var s:=1.35 if bool(monster.get("mvp",false)) else 1.0
		var p:=pos+Vector2(recoil,sin(clock*3+int(monster.get("level",1)))*2)
