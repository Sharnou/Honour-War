...
func _run() -> void:
    ...
    _require(game3d.find("func _try_attach_remote_production_asset(root:Node3D,class_id:String,level:int)->void:") >= 0,"remote native visual hook missing")
    _require(game3d.find('root.set_meta("hw_remote_native_actor",true)') >= 0,"remote players must use native Godot actor visuals")
    _require(game3d.find("HW_RemoteGeneratedGLB") < 0,"retired GLB remote-player path must remain absent")
