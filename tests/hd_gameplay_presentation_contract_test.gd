...
const LOOT_PROGRESSION = preload("res://scripts/LootProgressionSystem.gd")
...
func _initialize() -> void:
    ...
    check("Cicci equipment max two per kill", LOOT_PROGRESSION.clamp_equipment_drop_count(50) == 2)
    check("Cicci cards max two per kill", LOOT_PROGRESSION.clamp_card_drop_count(50) == 2)
    var no_bonus_hero:Dictionary = {"age_origin":18,"age":18,"online_days":0.0,"equipment":[],"cards":[]}
    var zero_roll:Dictionary = LOOT_PROGRESSION.roll_drop(0.0,no_bonus_hero,0.5)
    check("zero-rate loot can drop nothing", not bool(zero_roll.get("dropped",true)))
    var guaranteed_roll:Dictionary = LOOT_PROGRESSION.roll_drop(100.0,no_bonus_hero,0.0)
    ...
    var bonus_roll:Dictionary = LOOT_PROGRESSION.roll_drop(1.0,bonus_hero,0.08)
