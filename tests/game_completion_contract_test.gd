...
func _initialize() -> void:
    ...
    check("no active Barracks", CITY.BUILDINGS.is_empty() and not CITY.can_build(city_snapshot,"Barracks"))
    check("no active Tower Defense", not CITY.can_build(city_snapshot,"Tower Defense"))
    check("no active Soldier Production", not CITY.can_build(city_snapshot,"Soldier Production"))
    check("no active Guarded Bank", not CITY.can_build(city_snapshot,"Guarded Bank"))
    check("legacy city build disabled", not CITY.can_build(city_snapshot,"Blacksmith"))
    var unchanged:Dictionary = CITY.build(city_snapshot,"Blacksmith")
    check("legacy city build no-op", unchanged == city_snapshot)
    check("legacy city upgrade no-op", CITY.upgrade_city(city_snapshot) == city_snapshot)
