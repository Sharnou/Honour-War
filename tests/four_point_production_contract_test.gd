...
func _run() -> void:
    ...
    var visual_role:String = FileAccess.get_file_as_string("res://docs/DAILY_HONOUR_WAR_VISUAL_UPGRADE_ROLE.md")
    var neural_manifest:String = FileAccess.get_file_as_string("res://docs/NEURAL4D_REGENERATION_MANIFEST.md")
    _check("native production asset pipeline declared", art.contains("FBX/OBJ") and art.contains("native Godot 4.7.2"))
    _check("permanent no-GLB policy", no_glb_policy.contains("Status: **PERMANENT**") and no_glb_policy.contains("Meshy is **rejected"))
    _check("Neural4D regeneration manifest", neural_manifest.contains("Neural4D") and neural_manifest.contains("FBX") and neural_manifest.contains("OBJ"))
    _check("runtime native visual bridge enabled", project.contains("HWHDWorldContentDirector=") and project.contains("HWHDProductionCompleter="))
    _check("native world recovery enabled", project.contains("HWNativeWorldRecovery=") or FileAccess.file_exists("res://scripts/HWNativeWorldRecovery.gd"))
    _check("retired generated GLB library absent", _count_paths("res://assets/3d/generated") == 0)
